...
def set_last_msg_id(user_id, msg_id):
    db.fast_query('update users set last_msg_id=? where user_id=?', (msg_id, user_id))

def get_last_msg_id(user_id):
    data = db.select('select last_msg_id from users where user_id=?', (user_id,))
    if data:
        return data[0][0]
    else:
        None
def get_user_case(user_id):
    data = db.select('select case_id, omnidesk_id from users where user_id=?', (user_id,))
    if data[0]:
        return data[0]
    else:
        None
...
def set_user_case(user_id, case_id, omnidesk_id):
    db.fast_query('update users set case_id=?, omnidesk_id=? where user_id=?', (case_id, omnidesk_id, user_id))
